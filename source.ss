...
;; An entry is a pair of lists whose first list is a set. Also, the two lists
...
(define lookup-in-table
  (lambda (name table table-f)
    (cond
     ((null? table) (table-f name)) ; not in table.
     (else (lookup-in-entry name ; look in first entry.
                            (car table)
                            (lambda (name) ; advance if not found in first
                                           ; entry(!)
